...
- **Permissions matrix** (collection-level, server-evaluated): facilitator = full; editor = create/update/delete own + move any card, vote; viewer/frozen = read + vote only? → **decision: frozen users cannot vote either (freeze means freeze); viewers can vote on open polls is NOT supported v1 — vote requires editor.** Room data readable only by room members.
...
- **Payments**: Free = 3 imports total (counted server-side); Pro subscription = unlimited via `requireSubscription()`. Pricing page. (Real checkout blocked until Connect onboarding — acceptable; gate must still enforce.)
